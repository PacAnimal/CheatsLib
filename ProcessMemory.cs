...
namespace Nummer.Cheats
{
    internal class ProcessMemory
    {
        ...
        // Methods
        public ProcessMemory(uint ProcPID)
        {
            this.ProcessName = ProcPID.ToString();
        }
        ...
        {
            return BitConverter.ToDouble(this.ReadMem(pOffset, 4), 0);
        }

        public double ReadDouble(bool AddToImageAddress, int pOffset)
        {
            return BitConverter.ToDouble(this.ReadMem(pOffset, 4, AddToImageAddress), 0);
        }

        public double ReadDouble(string Module, int pOffset)
        {
            return BitConverter.ToDouble(this.ReadMem(this.DllImageAddress(Module) + pOffset, 4), 0);
        }
        ...
        public bool StartProcess()
        {
            if (this.ProcessName != "")
            {
                Process myprc = Process.GetProcessById(Convert.ToInt32(this.ProcessName));
                this.processHandle = OpenProcess(2035711, false, myprc.Id);
